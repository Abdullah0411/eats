...
import 'package:eats_core/services/google_geocode_api/google_geocode.dart';
import 'dart:convert';
...
class GoogleGeocodeServices {
  static AddressInformation getAddressFromGoogleGeocode(
      GoogleGeocodeResult googleGeocode) {
    final addressInformation = AddressInformation(
      street: googleGeocode.results?.first.addressComponents
          ?.firstWhereOrNull((addressComponent) =>
              addressComponent.types?.contains("route") ?? false)
          ?.longName,
      neighborhood: googleGeocode.results?.first.addressComponents
          ?.firstWhereOrNull((addressComponent) =>
              addressComponent.types?.contains("sublocality") ?? false)
          ?.longName,
      city: googleGeocode.results?.first.addressComponents
          ?.firstWhereOrNull((addressComponent) =>
              addressComponent.types?.contains("locality") ?? false)
          ?.longName,
      ...
      country: googleGeocode.results?.first.addressComponents
          ?.firstWhereOrNull((addressComponent) =>
              addressComponent.types?.contains("country") ?? false)
          ?.longName,
    );
  ...
  static Future<AddressInformation?> findLocationInformation(double latitude,
      double longitude, String googleApiKey, String? language) async {
    final params = {
