...
import 'package:eats_core/enums/amplitude_status.dart';
...
class AmplitudeServices {
  ...
  static void logEvent(
      String eventType, Map<String, dynamic> eventProperties) async {
    if (status == AmplitudeStatus.error) {
      // In case of an error in the init.
      // This should be changed from print statement to logger.
      GLogger.warning(
          '(Amplitude/logEvent) No API key passed when initializing Amplitude 🔴');
    } else {
      await ensureInitialized();
      await Amplitude.getInstance()
          .logEvent(eventType, eventProperties: eventProperties);
    }
  }
